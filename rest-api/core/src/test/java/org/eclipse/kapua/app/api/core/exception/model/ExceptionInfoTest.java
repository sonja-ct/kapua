...
public class ExceptionInfoTest extends Assert {
    ...
    public void initialize() {
        ...
        kapuaErrorCodes = new KapuaErrorCodes[]{KapuaErrorCodes.ADMIN_ROLE_DELETED_ERROR, KapuaErrorCodes.ENTITY_NOT_FOUND, KapuaErrorCodes.ENTITY_ALREADY_EXISTS, KapuaErrorCodes.DUPLICATE_NAME,
                ...
                KapuaErrorCodes.DOWNLOAD_PACKAGE_EXCEPTION, KapuaErrorCodes.TRIGGER_NEVER_FIRE, KapuaErrorCodes.PERMISSION_DELETE_NOT_ALLOWED,
        ...
        expectedErrorCodes = new String[]{"ADMIN_ROLE_DELETED_ERROR", "ENTITY_NOT_FOUND", "ENTITY_ALREADY_EXISTS", "DUPLICATE_NAME", "DUPLICATE_EXTERNAL_ID", "ENTITY_UNIQUENESS", "ILLEGAL_ACCESS", "ILLEGAL_ARGUMENT",
                ...
                "SUBJECT_UNAUTHORIZED", "ENTITY_ALREADY_EXIST_IN_ANOTHER_ACCOUNT", "EXTERNAL_ID_ALREADY_EXIST_IN_ANOTHER_ACCOUNT", "BUNDLE_START_ERROR", "BUNDLE_STOP_ERROR", "PACKAGE_URI_SYNTAX_ERROR", "MAX_NUMBER_OF_ITEMS_REACHED", "END_BEFORE_START_TIME_ERROR",
                "SAME_START_AND_DATE", "RETRY_AND_CRON_BOTH_SELECTED", "DEVICE_NOT_FOUND", "SCHEDULE_DUPLICATE_NAME", "DOWNLOAD_PACKAGE_EXCEPTION", "TRIGGER_NEVER_FIRE", "PERMISSION_DELETE_NOT_ALLOWED",
                "SERVICE_DISABLED"};
    }
    ...
    public void exceptionInfoNullHttpStatusTest() {
        for (KapuaErrorCode code : kapuaErrorCodes) {
            KapuaException kapuaException = new KapuaException(code);
            new ExceptionInfo(null, code, kapuaException);
        }
    }

    @Test(expected = NullPointerException.class)
    public void exceptionInfoNullErrorCodeTest() {
        for (Response.Status status : statusList) {
            KapuaException kapuaException = new KapuaException(KapuaErrorCodes.BUNDLE_START_ERROR);
            new ExceptionInfo(status, null, kapuaException);
        }
    }
    ...
    public void exceptionInfoNullExceptionTest() {
        for (KapuaErrorCode code : kapuaErrorCodes) {
            for (Response.Status status : statusList) {
                new ExceptionInfo(status, code, null);
            }
        }
    }
}